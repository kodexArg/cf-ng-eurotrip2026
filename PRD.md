...
A personal travel companion site for Gabriel & Vanesa's Europe trip — April 19 to May 10, 2026. Route: Santiago de Chile → Madrid → Barcelona → Palma de Mallorca → Londres → París (transit) → Roma → Madrid (escala nocturna) → Buenos Aires.
...
- Static route drawn between all cities in order: SCL → MAD → BCN → PMI → LON → PAR → ROM → MAD → EZE
...
- ✓ Eurostar 9001 London St Pancras → Paris Gare du Nord — May 5 05:57 BST → 09:20 CEST (~£49, not confirmed)
- ✓ TGV INOUI 6923 Paris Gare de Lyon → Lyon Saint-Exupéry TGV — May 5 17:13 → 19:07 CEST (1h54m)
- ✓ easyJet U2 4421 LYS → Roma Fiumicino (FCO) — May 5 20:20 → 21:55 CEST (carry-on only, buffer LYS 1h13m)
